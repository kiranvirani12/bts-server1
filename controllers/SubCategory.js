...
/**
 * Get list of sub-category for user
 *
 * @param {ClientRequest} req - The http request object
 * @param {IncomingMessage} res - The http response object
 * @param {function} next - The callback used to pass control to the next action/middleware
 ...
 * @param {function} next - The callback used to pass control to the next action/middleware
 */
module.exports.getSubCategoryWithCategoryId = function getSubCategoryWithCategoryId(req, res, next) {
  let subCategoryService = new SubCategoryService(req.Logger);
  subCategoryService.getSubCategoryWithCategoryId(req.swagger.params, res, next);
};
